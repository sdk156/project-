...
const finalworkbook_name = "finaldata_with_duplicates.csv"; // filename of final csv file with all trials of one particular day
...
let asyncFunction = async() => {
    ...
    console.log("final populated row should be: ", 3 * desired_row_count * filename.length + 1); //+1 is because, first row is a header
    for (let i = 0; i < filename.length; i++) {
        ...
        for (let j = 1; j <= 3; j++) { //each csv file has three times the shrimp ambience hence  1<=j<=3// refer the comments written at the beginning
            ...
            finalsheetRows_temp.forEach(function(value, rowIndex, rowArray) { // (current array element, index in finalsheetRows_temp, finalsheetRows_temp)
                ...
                // console.log("2", finalsheetRows_temp == rowArray);
                // h = false;
                // console.log("3, rowArray[rowIndex].values", rowArray[rowIndex].values);
                // rowArray[rowIndex].values = value.values.slice(1, 7).push(shrimpQualityClassificationInFileName[i]).push(dayCountFileName[i]);
                ...
                //inthe below code, before adding the row into the finalsheetAllRows array, duplicate checking is done and then addded into the it
                // rowArray[rowIndex] = [1, 2, 3, 4, 5].toString();
                ...
                fromRow_finalSheet++;
                // console.log("4", value);
                // console.log("4.1", value.values);
                // console.log("5, rowArray[rowIndex].values", rowArray[rowIndex]);
                // let temp_final_rows_array = value.values.slice(1, 7);
                // temp_final_rows_array.push(shrimpQualityClassificationInFileName[0]);
                // temp_final_rows_array.push(dayCountFileName[0]);
                // temp_final_rows_array = new Array(temp_final_rows_array);
                // console.log(temp_final_rows_array, "hii");
                // finalSheetAllRows.push(temp_final_rows_array);
                // finalSheetAllRows.push([1, 2, 3, 4, 5, 6, 7, 8]);
                // console.log(finalSheetAllRows);
                // console.log(m.);
                // }
                // let temp_final_rows_array = value.values.slice(1, 7);
                // temp_final_rows_array..push(shrimpQualityClassificationInFileName[i]).push(dayCountFileName[]);

                // rowArray[rowIndex] = value.values.slice(1, 7);

                // console.log('value', value);
                // console.log('rowArray[rowIndex]', rowArray[rowIndex]);
